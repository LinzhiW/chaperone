            ...
            <div className="box" style={{
              ...
              <div style={{ padding: "14px 18px", display: "flex", flexDirection: "column", gap: 12 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 12 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 4 }} data-comment-anchor="332620e387-div-325-19">
...
//   No slot cap. Equipped skills wrap. Library + saved sets share one panel via tabs.
function Skills_Loadout() {
  // Wide-ish equipped list to demo wrap + compact mode
  const equipped = ["css-th", "a11y", "tdd", "perf", "conv-cmt", "rest", "sec", "playwr"];
  return (
    <div className="wf" style={{ flexDirection: "row" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
        ...
        <div style={{ display: "flex", flex: 1, minHeight: 0 }}>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0, background: "var(--paper-2)" }}>
            ...
            {/* Single header row · breadcrumb + identity + mode hint */}
            <div style={{ padding: "10px 16px", borderBottom: "1.5px solid var(--rule)", background: "var(--paper-2)", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "var(--ink-3)" }}>
                  ...
                  <span className="mono">W1 · Dark Mode</span><span>›</span>
                  <span style={{ color: "var(--ink-2)", fontWeight: 600 }}>Loadout</span>
                </div>
                <div style={{ fontSize: 13, fontWeight: 700, marginTop: 1 }}>W1 · Dark Mode <span style={{ fontWeight: 400, color: "var(--ink-3)", fontSize: 11, marginLeft: 4 }}>ui-worker · ⎇ feat/dark-mode · ● running</span></div>
              </div>
              <span style={{ marginLeft: "auto", fontSize: 11, color: "var(--ink-3)" }}>
                Changes apply to W1's next tool call. Save to commit · discard to revert.
              </span>
            </div>

            <div className="wf-scroll" style={{ flex: 1, padding: 16, display: "flex", flexDirection: "column", gap: 14, minHeight: 0, overflow: "auto" }}>

              {/* Equipped — sticky-note styled frame · rounder + warmer bg distinguishes it from the library card below */}
              <section style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                  ...
                  <span style={{ fontSize: 11, color: "var(--ink-3)" }}>{equipped.length} skills · drag to reorder · ✕ to unequip</span>

                  {/* Unsaved indicator + save controls — inline at right */}
                  <span style={{ marginLeft: "auto", display: "inline-flex", alignItems: "center", gap: 8 }}>
                    <span style={{ fontSize: 10, color: "var(--review)", display: "inline-flex", alignItems: "center", gap: 4, fontWeight: 600 }}>
                      ...
                      2 unsaved
                      <span style={{ color: "var(--ink-3)", fontFamily: "var(--mono)", fontSize: 10, fontWeight: 400, marginLeft: 4 }}>+ perf-budget · − a11y-audit</span>
                    </span>
                    <button className="btn" style={{
                    ...
                    <button className="btn" style={{
                      fontSize: 11, padding: "5px 12px", borderRadius: 4,
                      background: "var(--paper)", color: "var(--pm)",
                      border: "1.5px solid var(--pm)", fontWeight: 600
                    }} title="Save the current 8 skills as a reusable set">⊞ Save as new set</button>
                    <button className="btn" style={{
                      fontSize: 12, padding: "5px 14px", borderRadius: 4,
                      background: "var(--approve)", color: "var(--paper)",
                      border: "1.5px solid var(--approve)", fontWeight: 700
                    }}>✓ Save · apply to W1</button>
                  </span>
                </div>

                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, alignItems: "stretch" }}>
                  {equipped.map((id, i) => <SkillChip key={i} skill={SKILL(id)} onRemove={() => {}} />)}
                  <div style={{
                    minWidth: 110, padding: "5px 10px", borderRadius: 99,
                    border: "1.5px dashed var(--rule-soft)",
                    color: "var(--ink-3)", fontFamily: "var(--hand)", fontSize: 13,
                    display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 4
                  }}>＋ drop skill here</div>
                </div>
              </section>

              {/* Library + saved sets — share one panel via tabs · keeps the frame count low */}
              <section className="box" style={{ background: "var(--paper)", display: "flex", flexDirection: "column", minHeight: 0, flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 4, padding: "0 12px", borderBottom: "1.5px solid var(--rule)" }}>
                  <LoadoutTab label="Library" active />
                  <LoadoutTab label="Apply saved set" badge="3" />
                  <div className="composer" style={{ marginLeft: "auto", margin: "6px 0", padding: "4px 8px", maxWidth: 220, fontSize: 11 }}>
                    <span style={{ fontSize: 11 }}>🔍 search library</span>
                  </div>
                </div>

                <div className="wf-scroll" style={{ flex: 1, overflow: "auto", padding: 12, display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(170px, 1fr))", gap: 8, alignContent: "start" }}>
        ...
        <BottomBar extra={`loadout · W1 · ${equipped.length} equipped · 2 unsaved`} />
      </div>
    </div>);

}

// Compact equipped-skill chip — name only, no category (library cards below carry the details)
function SkillChip({ skill, onRemove }) {
  ...
  return (
    <span style={{
      ...
      <span style={{ width: 5, height: 5, borderRadius: 99, background: skill.color }} />
      <span>{skill.name}</span>
      <span onClick={onRemove} title="unequip" style={{
        fontSize: 12, color: "var(--ink-3)", cursor: "pointer", padding: "0 2px",
        borderRadius: 99, lineHeight: 1
      }}>×</span>
    </span>);

}

function LoadoutTab({ label, active, badge }) {
  return (
    <div style={{
      padding: "8px 12px",
      ...
      <span>{label}</span>
      {badge && <span style={{ fontSize: 9, padding: "1px 5px", borderRadius: 99, background: "var(--paper-2)", color: "var(--ink-2)", fontFamily: "var(--mono)", fontWeight: 700 }}>{badge}</span>}
    </div>);
      ...
      <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
        ...
        <div style={{ display: "flex", flex: 1, minHeight: 0 }}>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0, background: "var(--paper-2)" }}>
            ...
            <div style={{ flex: 1, padding: 16, display: "flex", flexDirection: "column", gap: 12, minHeight: 0, overflow: "hidden" }} data-comment-anchor="a2756e819c-div-585-13">

              {/* Compose slot row */}
              <div className="box" style={{ padding: "14px 16px", background: "var(--paper)", display: "flex", flexDirection: "column", gap: 10 }} data-comment-anchor="b99692ec42-div-588-15">
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
...
Object.assign(window, { Skills_Library, Skills_Import, Skills_Loadout, Skills_ComposeSet, Sidebar_Skills, SourceFilter, SortChip, LibrarySkillCard, ImportTab, ChoiceRow, SkillChip, LoadoutTab });